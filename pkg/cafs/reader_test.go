...
func TestWriteTo(t *testing.T) {
	...
	fStore.chunks[keyStr1] = internal.RandBytesMaskImprSrc(64 * 1024)
	fStore.chunks[keyStr2] = internal.RandBytesMaskImprSrc(64 * 1024)
	...
	require.NoError(t, err)
	keys := []Key{key1, key2}
	reader, err := newReader(&fStore, key, 64*1024, "",
		TruncateLeaf(false),
		Keys(keys),
	)
	require.NoError(t, err)
	...
	require.Equal(t, fStore.chunks[keyStr1], fakeWriterAt.data[:64*1024])
	require.Equal(t, fStore.chunks[keyStr2], fakeWriterAt.data[64*1024:])
	...
	require.NoError(t, err)
	require.Equal(t, written, int64(2*64*1024))
	require.Equal(t, fStore.chunks[keyStr1], fakeWriter.data[:64*1024])
	require.Equal(t, fStore.chunks[keyStr2], fakeWriter.data[64*1024:])
